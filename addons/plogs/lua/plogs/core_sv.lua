...
plogs.playerLogCache = plogs.playerLogCache or {}

function plogs.PlayerLog(pl, type, str, copy)
	local currentTime = CurTime()
	local cacheKey = IsValid(pl) and pl:SteamID() or "console"
	
	local cache = plogs.playerLogCache[cacheKey]
	if not cache then
		cache = {
			lastMessage = nil,
			lastTime = 0
		}
		plogs.playerLogCache[cacheKey] = cache
	end
	
	if cache.lastMessage == str and (currentTime - (cache.lastTime or 0)) < 2 then
		return
	end
	
	cache.lastMessage = str
...
function plogs.FindPlayer(info)
	...
	for k, v in ipairs(player.GetAll()) do
		if (v:SteamID() == info) or (v:SteamID64() == info) or (v:Nick() == info) then
			return v
		end
	end
end
