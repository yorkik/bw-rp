...
local check_updates = function()
	sam.print("Connecting to database...")

	-- SQL.Query([[DROP TABLE IF EXISTS `sam_players`]]):wait()
	-- SQL.Query([[DROP TABLE IF EXISTS `sam_bans`]]):wait()
	-- SQL.Query([[DROP TABLE IF EXISTS `sam_ranks`]]):wait()
	-- SQL.Query([[DROP TABLE IF EXISTS `sam_version`]]):wait()
	-- SQL.Query([[DROP TABLE IF EXISTS `sam_config`]]):wait()
...
-- fix the issue with the sam_config.adverts being corrupted because of messagepack being weird
add_version(154, function()
	SQL.Query([[
		SELECT
			`key`,
			`value`
		FROM
			...
			for _, v in ipairs(sam_config) do
				local status, config_value = pcall(sam.mp.unpack, v.value)
				if not status then
					sam.print("Failed to decode sam_config value for key: \"" .. v.key .. "\" skipping...")
				else
					SQL.FQuery([[
						UPDATE
							`sam_config`
						...
						WHERE
							`key` = {2}
					]], {sam.to_hex(sam.sfs.encode(config_value)), v.key})
				end
			end
			SQL.Commit(nil, true)
		end
