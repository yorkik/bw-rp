...
local err_mat = SUI.Material("___error", nil, "")
...
function PANEL:SetImage(url)
	...
	if url:sub(1, 4) ~= "http" then
		self.image = SUI.Material(url, nil, "noclamp")
		return
	end
	...
	if is_gif then
		...
		self.image = SUI.Material("../data/" .. image_path, nil, "noclamp")
	end
end
...
function PANEL:PaintGIF(w, h, images)
	...
	local mat = SUI.Material(images[frame], true, "noclamp")
	if not mat then
		if frame > 1 then
			mat = SUI.Material(images[frame - 1], nil, "noclamp")
		else
			mat = err_mat
		end
