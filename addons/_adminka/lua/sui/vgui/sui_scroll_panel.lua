...
local vbar_SetScroll = function(s, amount)
	...
	s.scroll_target = math.Clamp(amount or 0, 0, s.CanvasSize)
	s:InvalidateLayout()
end
...
local vbar_OnCursorMoved = function(s, _, y)
	...
	local track = (s:GetTall() - s:GetWide() * 2 - s.btnGrip:GetTall())
	if track <= 0 then return end

	local frac = math.Clamp(y / track, 0, 1)
	s.scroll_target = frac * s.CanvasSize
end

local vbar_Think = function(s)
	local t = math.min(RealFrameTime() * 15, 1)

	if not s.Dragging then
		s.scroll_target = math.Clamp(s.scroll_target or 0, 0, s.CanvasSize)
	end

	local old = s.Scroll or 0
	local target = s.scroll_target or old

	s.Scroll = math.Clamp(Lerp(t, old, target), 0, s.CanvasSize)

	if s.Scroll ~= old then
		s:InvalidateLayout(false)
	end

	s.scroll_speed = Lerp(t * 0.1, s.scroll_speed or starting_scroll_speed, starting_scroll_speed)
end
